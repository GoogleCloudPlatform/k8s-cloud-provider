...
func TestCycleCheck(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := cycleCheck(Path{}, tc.t, []string{})
			gotErr := err != nil
...
func TestVersionedObjectCheck(t *testing.T) {
	type innerSt struct{}
	type okSt struct {
		ST  innerSt
		PST *innerSt
	}
	type badSt struct{ C chan int }
