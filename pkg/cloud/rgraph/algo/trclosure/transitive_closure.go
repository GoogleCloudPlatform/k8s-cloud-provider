	...
	fn := func(ctx context.Context, w work) error {
		...
		for _, ref := range outRefs {
			...
			if gr.Get(ref.To) != nil {
				...
				klog.Infof("outRef %+v is already in the graph, ignoring", ref)
				graphLock.Unlock()
				continue
			}
			...
			klog.Infof("outRef %+v has not been traversed, adding to graph", ref)
			gr.Add(toNode)
			graphLock.Unlock()
...
func syncNode(ctx context.Context, cl cloud.Cloud, config Config, b rnode.Builder) ([]rnode.ResourceRef, error) {
	...
	klog.Infof("node.SyncFromCloud(%s) = %v (%s)", b.ID(), err, pretty.Sprint(b))
	...
	if b.State() != rnode.NodeExists {
		klog.Infof("Node %s resource state = %s, no outRefs", b.ID(), b.State())
		return nil, nil
	}

	if b.Ownership() == rnode.OwnershipExternal {
		// Nodes that are ExternallyOwned are not traversed for their references.
		klog.Infof("Node %s externally owned, no outRefs", b.ID())
		return nil, nil
	}
